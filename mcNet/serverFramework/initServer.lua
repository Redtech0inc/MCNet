...
local isStartup = true -- if true will start up with server
local serverPath = "server.lua" --this is the directory which will be executed on startup
...
function run()
    if not isStartup then return end
    ...
    term.setTextColor(colors.white)
    loading_bar(findCenter(nil,19),findCenter(sizeY),19," ",1,3,"Initiation Done!",2)

    shell.run("bg",serverPath)
